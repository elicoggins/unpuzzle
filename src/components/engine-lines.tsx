...
export function EngineLines({ fen, lines, depth, isSearching }: EngineLinesProps) {
        ...
        {displayLines.map((line, i) => {
          ...
          return (
            <div key={i} className="flex items-start gap-2 px-3 py-1.5">
              ...
              <span className="text-xs font-[family-name:var(--font-mono)] text-text-secondary leading-relaxed truncate">
                {line.sanMoves.length > 0
                  ? line.sanMoves.slice(0, 8).join("  ")
                  : "..."}
              </span>
            </div>
          );
